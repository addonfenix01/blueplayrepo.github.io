...
if not xbmcvfs.exists(profile):
    try:
        ...
        pass
FONTE = 'https://github.com/BluePlay8486/BluePlayHD/raw/refs/heads/main/BluePlay/Menu_Principal.xml'
try:
...
def get_text_settings(key):
    return addon.getSetting(key)

def set_text_settings(key,value):
    addon.setSetting(id=key, value=value)
...
def acess():
    try:
        username = base64.b64encode(get_text_settings('username').encode()).decode('utf-8')
        password = base64.b64encode(get_text_settings('password').encode()).decode('utf-8')        
        url = 'https://blueplay.alwaysdata.net/?action=acesso&username=%s&password=%s'%(urllib_parse.quote_plus(username),urllib_parse.quote_plus(password))
        src = makeRequest(url)
        if src:
            try:
            ...
            if src == 'ativado':
                try:
                    ping = 'https://blueplay.alwaysdata.net/ping/ping.php'
                    r = makeRequest(ping)
                except:
                    pass
                return True
        ...
        pass
    xbmcgui.Dialog().ok(addon_name, 'Login invalido ou expirado')    
    return False 

# VIP
def info_acc():
    if get_text_settings('username') and get_text_settings('password'):
        try:
            username = base64.b64encode(get_text_settings('username').encode()).decode('utf-8')
            password = base64.b64encode(get_text_settings('password').encode()).decode('utf-8')
            url = 'https://blueplay.alwaysdata.net/?action=vencimento&username=%s&password=%s'%(urllib_parse.quote_plus(username),urllib_parse.quote_plus(password))
            src = makeRequest(url)
            if src:
                try:
                    src = src.decode('utf-8')
                    ...
                    pass
                xbmcgui.Dialog().ok(addon_name, 'Usuario: %s\nSenha: %s\nExpira: %s'%(get_text_settings('username'), get_text_settings('password'), src))
        except:
            xbmcgui.Dialog().ok(addon_name, 'Falha do servidor, avise um administrador')   

# VIP
def login_save():
    if not get_text_settings('username') and not get_text_settings('password'):
        username_ = input_text('Insira seu usuario')
        password_ = input_text('Insira sua senha')
        if username_ and password_:
            set_text_settings('username', username_)
            set_text_settings('password', password_)
...
def getSources():
    try:
        #if os.path.exists(favorites):
        if FAV:
            addDir('[B][COLOR yellow]●[/COLOR] [COLOR white]FAVORITOS[/COLOR][/B]', 'url', 4, os.path.join(home, 'resources', 'favorite.png'), FANART, '', '', '', '')
...
def addDir(name, url, mode, iconimage, fanart, description, genre, date, credits, showcontext=False, regexs=None, reg_url=None, allinfo={}):
    ...
    if showcontext:
        ...
        if showcontext == 'source':
            ...
            contextMenu.append(('Remover de '+NAME_TO_FAV+' Favoritos', 'RunPlugin(%s?mode=6&name=%s)'
        ...
        if str(name) not in FAV:
            try:
                url_fav = base64.b64encode(url.encode("utf-8")).decode('utf-8')
            except:
                url_fav = base64.b64encode(url)            
            contextMenu.append(('Adicionar a '+NAME_TO_FAV+' Favoritos', 'RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
                               % (sys.argv[0], urllib_parse.quote_plus(name), urllib_parse.quote_plus(url_fav), urllib_parse.quote_plus(iconimage), urllib_parse.quote_plus(fanart), mode)))
        ...
        if showcontext == 'fav':
            ...
            try:
                fav_params = (
                    ...
                    % (sys.argv[0], urllib_parse.quote_plus(name), urllib_parse.quote_plus(url_fav), urllib_parse.quote_plus(iconimage), urllib_parse.quote_plus(fanart))
                )
            except:
                fav_params = (
                    '%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=0'
                    % (sys.argv[0], urllib_parse.quote_plus(name), urllib_parse.quote_plus(url_fav),
            ...
            contextMenu.append(('Adicionar '+NAME_TO_FAV+' Favoritos', 'RunPlugin(%s)' % fav_params))
        liz.addContextMenuItems(contextMenu)
    try:
...
if mode is None:
    ...
    addon_log("LOGIN MENU")
    addDir('::: ENTRAR ::::', '', 62, icon, FANART, 'LOGIN', '', '', '')
    #getSources()
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
# VIP
    ...
    if acess():
        addon_log("getSources")
        addDir('::: OLHAR VENCIMENTO ::::', '', 63, icon, FANART, 'VENCIMENTO', '', '', '')
        getSources()
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
